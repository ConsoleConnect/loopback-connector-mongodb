    ...
    if (spec) {
      ...
      if (cond === null) {
        ...
        if (self.isObjectIDProperty(model, prop, cond, options)) {
          ...
          if (isPropStoredAsString(prop)) {
            cond = {'$in': [cond, cond.id.toString('hex')]};
          }
        }
...
function isPropStoredAsString(propDef) {
  return propDef && propDef.mongodb && propDef.mongodb.dataType === 'string';
};
